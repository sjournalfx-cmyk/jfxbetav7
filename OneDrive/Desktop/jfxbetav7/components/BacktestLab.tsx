...
const BacktestLab: React.FC<BacktestLabProps> = ({ isDarkMode, userProfile, onUpdateProfile }) => {
    ...
    const onMouseDown = (e: React.MouseEvent) => {
        ...
        let drawingProps: any = {
            ...
            ...(toolDefaults[activeTool] || { color: '#2962ff', strokeWidth: 1, strokeStyle: 'solid' })
        };
        ...
        <div className={`w-full h-full flex flex-col overflow-hidden font-sans ${isDarkMode ? 'bg-[#09090b] text-zinc-200' : 'bg-white text-slate-900'}`} onMouseUp={handleMouseUp}>
            ...
            {activeLabTab === 'chart' && (
                <div className={`h-16 shrink-0 border-t flex items-center px-8 justify-between relative z-20 ${isDarkMode ? 'border-zinc-800 bg-[#09090b]' : 'border-slate-200 bg-white'}`}>
                    ...
                    <div className="flex items-center gap-4">
                        {(isSticky || isLocked) && (
                            <div className="flex items-center gap-2">
                                {isSticky && <span className="px-2 py-1 rounded bg-amber-500/20 text-amber-500 text-[8px] font-black uppercase tracking-widest">STICKY</span>}
                                {isLocked && <span className="px-2 py-1 rounded bg-rose-500/20 text-rose-500 text-[8px] font-black uppercase tracking-widest">LOCKED</span>}
                            </div>
                        )}
            ...
            {isSettingsOpen && (
                <div className="fixed inset-0 z-[400] flex justify-end">
                    ...
                    <div className={`relative w-full max-w-sm h-full shadow-2xl border-l flex flex-col ${isDarkMode ? 'bg-[#09090b] border-zinc-800' : 'bg-white border-slate-200'}`}>
                        {/* Drawer Header */}
            ...
            {editingDrawingSettings && (
                <div className="absolute bottom-20 left-1/2 -translate-x-1/2 z-[100] w-full max-w-xs pointer-events-none">
                    <div className={`pointer-events-auto rounded-lg border shadow-2xl overflow-hidden bg-black text-white border-zinc-800/80 backdrop-blur-md`}>
                        <div className={`flex items-center justify-between px-5 py-3 border-b border-zinc-800/50 bg-zinc-900/40`}>
                            <div className="flex items-center gap-2">
                                <Settings size={12} className="text-[#FF4F01]" />
                                <h3 className="text-[9px] font-black uppercase tracking-[0.2em] opacity-80">Drawing Settings</h3>
                            </div>
                            <button onClick={() => setEditingDrawingSettings(null)} className="p-1 hover:bg-zinc-800 rounded-md transition-colors text-zinc-400 hover:text-white"><X size={14} /></button>
                        </div>

                        <div className="p-5 grid grid-cols-2 gap-6">
                            {/* Left Column: Color & Style */}
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <label className="text-[8px] font-black uppercase tracking-wider text-zinc-500 block">Color</label>
                                    <div className="grid grid-cols-4 gap-1.5">
                                        {['#2962ff', '#FF4F01', '#10b981', '#ef4444', '#f59e0b', '#8b5cf6', '#ec4899', '#ffffff'].map(c => (
                                            <button 
                                                key={c} 
                                                onClick={() => updateDrawingProperty(editingDrawingSettings.id, { color: c })} 
                                                className={`w-5 h-5 rounded border transition-all ${editingDrawingSettings.color === c ? 'border-[#FF4F01] scale-110 shadow-sm' : 'border-zinc-800 hover:border-zinc-500'}`} 
                                ...
                                <div className="space-y-2">
                                    <label className="text-[8px] font-black uppercase tracking-wider text-zinc-500 block">Style</label>
                                    <div className="flex gap-1">
                                        {['solid', 'dashed', 'dotted'].map(s => (
                                            <button 
                                                key={s} 
                                                onClick={() => updateDrawingProperty(editingDrawingSettings.id, { strokeStyle: s as any })} 
                                                className={`flex-1 py-1 rounded text-[8px] font-bold uppercase transition-colors ${editingDrawingSettings.strokeStyle === s || (!editingDrawingSettings.strokeStyle && s === 'solid') ? 'bg-[#FF4F01] text-white' : 'bg-zinc-900/80 text-zinc-400 hover:text-zinc-200'}`}
                                            >
                                                {s[0]}
                                            </button>
                                        ))}
                            ...
                            {/* Right Column: Width & Remove */}
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center">
                                        <label className="text-[8px] font-black uppercase tracking-wider text-zinc-500">Width</label>
                                        <span className="text-[9px] font-mono font-bold opacity-60">{editingDrawingSettings.strokeWidth || 1}px</span>
                                    </div>
                                    <input
                                        type="range" min="1" max="4" step="1"
                                        value={editingDrawingSettings.strokeWidth || 1}
                                        onChange={(e) => updateDrawingProperty(editingDrawingSettings.id, { strokeWidth: parseInt(e.target.value) })}
                                        className="w-full h-1 bg-zinc-800 appearance-none cursor-pointer accent-[#FF4F01]"
                                    />
                                </div>
                                <div className="pt-2">
                                    <button 
                                        onClick={() => { 
                                            setDrawings(drawings.filter(d => d.id !== editingDrawingSettings.id)); 
                                            pushToHistory(drawings.filter(d => d.id !== editingDrawingSettings.id)); 
                                            setEditingDrawingSettings(null); 
                                            setSelectedDrawingId(null); 
                                        }} 
                                        className="w-full py-2 bg-rose-500/5 hover:bg-rose-500 text-rose-500 hover:text-white border border-rose-500/10 rounded text-[8px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-1.5"
                                    >
