...
import {
  ...
  Moon, Copy, Check, Crown, HelpCircle,
  MessageSquare, Video, LifeBuoy, Medal,
  Send, ChevronDown, Code, Send as Telegram,
  LogOut
} from 'lucide-react';
...
interface SettingsProps {
  ...
  migrationVersion?: string; // New prop for migration version
}
        ...
        <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-900/50 border-zinc-800' : 'bg-slate-50 border-slate-200'}`}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] opacity-50">End your session securely</p>
            </div>
          </div>
          <button
            onClick={onLogout}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm text-white bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 transition-all shadow-lg shadow-rose-500/20 hover:shadow-rose-500/30"
          >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className={`p-2 rounded-lg transition-all hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-500`}
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex flex-col gap-6">
                  <div className="flex items-center justify-between p-6 rounded-2xl border border-dashed border-zinc-800">
                    <div className="flex items-center gap-4">
                      <div className="p-3 rounded-xl bg-[#FF4F01]/10 text-[#FF4F01]">
                        <Moon size={24} />
                      </div>
                      <div>
                        <p className="text-lg font-bold">Display Mode</p>
                        <p className="text-sm opacity-50">Obsidian mode is locked on for this workspace.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {formData.plan === 'PREMIUM (MASTERS)' && (
                    <div className={`p-8 rounded-3xl border ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {[
                          { id: 'obsidian', label: 'Obsidian', desc: 'Standard dark aesthetic.', bg: 'bg-[#050505]', accent: 'bg-[#FF4F01]' },
                          { id: 'cosmic', label: 'Cosmic', desc: 'Deep contrast with violet edges.', bg: 'bg-[#030712]', accent: 'bg-violet-500' },
                        ].map((t) => (
                          <button
                            key={t.id}
                ...
                {migrationVersion && (
                  <div className={`p-4 rounded-2xl border border-dashed flex items-center justify-between ${isDarkMode ? 'bg-zinc-900/30 border-zinc-800' : 'bg-slate-100 border-slate-200'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold">Database Migration Version</p>
                        <p className="text-[10px] opacity-50">Currently applied: {migrationVersion}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      { title: 'Video Tutorials', icon: Video, color: 'text-red-500', desc: 'Watch step-by-step guides.', link: null },
                      ...
                      { title: 'GitHub Issues', icon: MessageSquare, color: 'text-zinc-500', desc: 'Report bugs & feature requests.', link: null },
                    ].map((item, i) => {
