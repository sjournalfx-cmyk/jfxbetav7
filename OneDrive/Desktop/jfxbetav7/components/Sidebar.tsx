...
import {
  ...
  Settings, Link, Activity, Lock, Bell, X, CheckCircle2, AlertCircle, AlertTriangle, Info, Trash2, Globe, Medal
} from 'lucide-react';
import {
  IconPower,
  IconBell,
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const systemItems = [
    ...
    { id: 'logout', icon: <IconPower size={18} className="text-rose-500" />, label: 'Log Out', description: 'Sign out of your account', onClick: onLogout, variant: 'danger' },
  ];
  ...
  return (
    ...
    >
      ...
      <motion.div 
        layout="position"
        className="flex items-center w-full mb-4 px-[18px] shrink-0 justify-start"
      >
        <div className={`w-9 h-9 rounded-full border flex items-center justify-center relative shrink-0 transition-colors duration-500 ${isBridgeOnline ? 'border-emerald-500/30' : 'border-rose-500/30'}`}>
          {isBridgeOnline ? (
